...
def obtener_curso(db: DbSession, id_curso: UUID) -> CursoResponse:
    ...
    if not db_curso:
        raise HTTPException(
            ...
            detail=f"El curso con ID {id_curso} no existe en la base de datos",
        )
    return db_curso
...
def crear_curso(
    db: DbSession, dato: CursoCreate, user_id: str = Depends(get_current_user_id)
):
...
def actualizar_curso(
    ...
    curso = services_curso.actualizar(
        db, id_curso, id_usuario_edita=user_id, **dato.model_dump(exclude_unset=True)
    )
...
def eliminar_curso(
    db: DbSession, id_curso: UUID, user_id: str = Depends(get_current_user_id)
) -> None:
    try:
        ...
        if curso_existente.id_usuario_creacion != UUID(user_id):
            raise HTTPException(
                status_code=403, detail="No autorizado para eliminar este curso"
            )
